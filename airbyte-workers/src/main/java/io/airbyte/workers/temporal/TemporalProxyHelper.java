...
public class TemporalProxyHelper {
   ...
   */
  @Inject
  private Collection<BeanRegistration<ActivityOptions>> activityOptions;

  /**
   * Function used to generate Temporal activity stubs.
   */
  private Optional<ActivityStubFunction<Class<?>, ActivityOptions, Object>> activityStubGenerator = Optional.empty();

  public TemporalProxyHelper(final Collection<BeanRegistration<ActivityOptions>> activityOptions) {
  ...
  public <T> Class<T> proxyWorkflowClass(final Class<T> workflowImplClass) {
    ...
    return (Class<T>) WORKFLOW_PROXY_CACHE.findOrInsert(workflowImplClass.getClassLoader(), workflowImplClass, () -> {
      ...
      final Class<T> type = (Class<T>) new ByteBuddy()
          ...
          .intercept(
              MethodDelegation.to(generateInterceptor(workflowImplClass, activityOptions)))
          .make()
